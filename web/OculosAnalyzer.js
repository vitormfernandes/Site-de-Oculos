...
const faceMesh = new FaceMesh({
    // CORRIGIDO: O URL da biblioteca estava com um erro de digitação.
...
// --- Função chamada a cada quadro de vídeo processado ---
...
function drawEyeMarkers(landmarks) {
    ...
    const leftX = (1 - leftEye.x) * debugCanvas.width;
    const leftY = leftEye.y * debugCanvas.height;
    const rightX = (1 - rightEye.x) * debugCanvas.width;
    const rightY = rightEye.y * debugCanvas.height;
    
    debugCtx.fillStyle = 'cyan';
    
    debugCtx.beginPath();
    debugCtx.arc(leftX, leftY, 3, 0, 2 * Math.PI);
    debugCtx.fill();
    
    debugCtx.beginPath();
    debugCtx.arc(rightX, rightY, 3, 0, 2 * Math.PI);
    debugCtx.fill();
}
...
function drawNoseMarker(landmarks) {
    ...
    const noseX = (1 - noseTip.x) * debugCanvas.width;
    const noseY = noseTip.y * debugCanvas.height;

    debugCtx.fillStyle = 'orange';
    
    debugCtx.beginPath();
    debugCtx.arc(noseX, noseY, 3, 0, 2 * Math.PI);
    debugCtx.fill();
}
...
function positionGlasses(landmarks) {
    ...
    // AUMENTADO: O multiplicador de escala foi aumentado para os óculos parecerem maiores.
    // Você pode ajustar este valor (380) se quiser ainda maior ou um pouco menor.
...
// --- Lógica do botão "Testar no Rosto" ---
tryOnBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        alert("Não foi possível acessar a câmera. Por favor, verifique se você deu a permissão necessária no seu navegador.");
    }
});
...
function setupThreeJsOverlay() {
    ...
    overlayCamera = new THREE.OrthographicCamera(
        overlayCanvas.width / -2, overlayCanvas.width / 2,
        overlayCanvas.height / 2, overlayCanvas.height / -2,
        1, 1000
    );
    ...
    loader.load('/assets/Modelos_Oculos/oculos1.stl', (geometry) => {
